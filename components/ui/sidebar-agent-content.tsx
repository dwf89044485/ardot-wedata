...
const FOLDER_TASKS_DEV: TaskPreviewData[] = [
  ...
  },
  {
    ...
    time: "3天前",
    statusLabel: "已完成",
    artifacts: [{ name: "血缘链路图", tag: "4 层 · 12 节点" }],
  ...
  },
  {
    ...
    time: "5天前",
    statusLabel: "已完成",
    artifacts: [{ name: "workflow_daily_etl", tag: "5 个节点" }],
...
const FOLDER_TASKS_TEST: TaskPreviewData[] = [
  ...
  },
  {
    ...
    time: "前天",
    statusLabel: "已完成",
    artifacts: [{ name: "new_user_by_channel_30d.sql" }],
...
function TaskPreviewTooltip({ data, anchorRect }: {
  ...
  return (
    ...
    >
      ...
      {/* Prompt quote card */}
      <div style={{ background: "#F2F4F8", borderRadius: 8, padding: 10 }}>
        <div style={{ display: "flex", alignItems: "stretch" }}>
          <div style={{ flexShrink: 0, display: "flex", flexDirection: "column", alignItems: "flex-start", paddingRight: 6 }}>
            <QuoteOpen />
          </div>
          <div style={{ flex: 1, display: "flex", alignItems: "center", minWidth: 0 }}>
            <div style={{
              fontSize: 14, lineHeight: "19.6px", fontWeight: 400, color: "rgba(0,0,0,0.9)",
          ...
          <div style={{ flexShrink: 0, display: "flex", flexDirection: "column", justifyContent: "flex-end", alignItems: "flex-end", paddingLeft: 6 }}>
            <QuoteClose />
          </div>
        </div>
      </div>
      ...
      {data.turnNote && (
        <div style={{ fontSize: 12, lineHeight: "16px", color: "rgba(0,0,0,0.35)", marginTop: -8, paddingLeft: 2 }}>
          {data.turnNote}
        </div>
      )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              {data.steps.map((step, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "3px 0" }}>
                  <div style={{
                    ...
                    background: step.state === "done" ? "#00B6C3" : step.state === "active" ? "#00B6C3" : "#D9DCE1",
                    boxShadow: step.state === "active" ? "0 0 0 3px rgba(0,182,195,0.2)" : "none",
                  }} />
                  <span style={{
                    fontSize: 13, lineHeight: "18px",
                    color: step.state === "done" ? "rgba(0,0,0,0.4)" : step.state === "active" ? "rgba(0,0,0,0.9)" : "rgba(0,0,0,0.3)",
          ...
          {data.status !== "failed" && data.artifacts && data.artifacts.length > 0 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              <span style={{ fontSize: 12, fontWeight: 500, lineHeight: "16px", color: "rgba(0,0,0,0.45)" }}>
                产物
              </span>
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                {data.artifacts.map((a, i) => (
                  <div key={i} style={{
                    ...
                    padding: "4px 8px", background: "rgba(0,0,0,0.03)", borderRadius: 6,
                  }}>
                    ...
                    {a.tag && (
                      <span style={{
                        ...
                        padding: "1px 6px", background: "rgba(0,0,0,0.04)", borderRadius: 4,
          ...
          {data.resultSummary && (
            <div style={{ fontSize: 13, lineHeight: "18px", color: "rgba(0,0,0,0.55)", paddingLeft: 2 }}>
              {data.resultSummary}
            </div>
          )}
          ...
          {data.reason && !data.steps && (
            <div style={{
              ...
              borderRadius: 6, whiteSpace: "pre-line",
            }}>
              {data.reason}
            </div>
          )}
...
function SectionHeader({ label, withTopBorder = false }: { label: string; withTopBorder?: boolean }) {
  return (
    <div style={{
      padding: "24px 12px 8px", width: "100%",
      borderTop: withTopBorder ? `1px solid ${C.borderColor}` : "none",
    }}>
      <span style={{ fontFamily: FONT, fontSize: 12, fontWeight: 600, lineHeight: "20px", color: C.textSecondary }}>
        {label}
      </span>
    </div>
  );
...
// ── Task item ──────────────────────────────────────────────────
function TaskItem({ status, title, preview }: {
  status: TaskStatus;
  title: string;
    ...
    <>
      <div
        ...
        style={{
          ...
          paddingLeft: 12, paddingRight: 12, borderRadius: 20, cursor: "pointer",
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 8, flex: "1 0 0", minWidth: 0 }}>
          ...
          <span style={{
            fontFamily: FONT, fontSize: 14, fontWeight: 400, lineHeight: "22px", color: C.textPrimary,
            overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: "1 0 0", minWidth: 0,
          }}>
            {title}
          </span>
        </div>
...
function MoreLink({ count }: { count: number }) {
  ...
  return (
    ...
    >
      <span style={{ fontFamily: FONT, fontSize: 14, fontWeight: 400, lineHeight: "20px", color: C.textTertiary }}>
        显示更多({count})
      </span>
    </div>
  );
...
export default function SidebarAgentContent({ onNewTask }: SidebarAgentContentProps) {
      ...
      <div style={{ padding: "0 12px 8px", flexShrink: 0 }}>
        <button
          ...
          style={{
            ...
            padding: "8px 20px", outline: "none", transition: "background 100ms",
          }}
        >
          <IconAiNewChat />
          <span style={{
            fontFamily: FONT, fontSize: 14, fontWeight: 500, lineHeight: "22px", color: C.textPrimary,
            overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
          }}>
            新建任务
          </span>
